...
describe('NavBar component', () => {
  ...
  it('should render navigation links', async () => {
    ...
    await waitFor(() => screen.getAllByText('Jugar'));

    const playLinks = screen.getAllByText('Jugar');
    await expect(playLinks[0]).toBeInTheDocument();
    await expect(playLinks[1]).toBeInTheDocument();

    const statisticsLinks = screen.getAllByText('Estadísticas');
    await expect(statisticsLinks[0]).toBeInTheDocument();
    await expect(statisticsLinks[1]).toBeInTheDocument();


    const instructionsLinks = screen.getAllByText('Instrucciones');
